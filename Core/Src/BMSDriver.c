                   ...
                   )
{
    ...
    {
        // Transmit byte.
        HAL_SPI_TransmitReceive(pspi, (uint8_t*)&tx_data[i], (uint8_t*)&rxDummy, 1, HAL_MAX_DELAY);
    }
    ...
    {
        // Receive byte.
        HAL_SPI_TransmitReceive(pspi, (uint8_t*)&txDummy, (uint8_t*)&rx_data[i], 1, HAL_MAX_DELAY);
    }
}
...
uint8_t spi_read_byte(uint8_t tx_data)
{
	SPI_HandleTypeDef *pspi=&hspi3;
	uint8_t rx_data;
    if ( HAL_SPI_TransmitReceive(pspi, (uint8_t*) &tx_data, (uint8_t*)&rx_data, 1, HAL_MAX_DELAY) == HAL_OK )
    {
        return(rx_data);
    }
	return(1);
